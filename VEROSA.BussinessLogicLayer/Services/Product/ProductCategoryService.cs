...
namespace VEROSA.BussinessLogicLayer.Services.Product
{
    public class ProductCategoryService : IProductCategoryService
    {
        ...
        public async Task<IEnumerable<ProductCategoryResponse>> GetAllAsync()
        {
            try
            {
                var list = await _unitOfWork.ProductCategories.GetAllAsync();
        ...
        public async Task<ProductCategoryResponse?> GetByIdAsync(Guid id)
        {
            try
            {
                var e = await _unitOfWork.ProductCategories.GetByIdAsync(id);
                if (e == null)
                    return null;
        ...
        public async Task<bool> UpdateAsync(Guid id, ProductCategoryRequest req)
        {
            try
            {
                ...
                if (e == null)
                    return false;
                _mapper.Map(req, e);
                _unitOfWork.ProductCategories.Update(e);
                await _unitOfWork.CommitAsync();
                return true;
            }
            catch (Exception ex)
        ...
        public async Task<bool> DeleteAsync(Guid id)
        {
            try
            {
                ...
                if (e == null)
                    return false;
                _unitOfWork.ProductCategories.Delete(e);
                await _unitOfWork.CommitAsync();
                return true;
            }
            catch (Exception ex)
        ...
        )
        {
            try
            {
                var all = (await _unitOfWork.ProductCategories.FindCategoriesAsync(name)).ToList();
                if (!all.Any())
